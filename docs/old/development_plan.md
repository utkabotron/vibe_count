...
## Этап 9: Деплой в Timeweb Cloud
**Цель:** Развернуть систему в продакшн на VPS

### Задачи:
1. **Подготовка VPS**
   - [ ] Заказать VPS на Timeweb (минимальная конфигурация: 1 CPU, 1GB RAM)
   - [ ] Настроить SSH доступ
   - [ ] Обновить систему: `apt update && apt upgrade`
   - [ ] Установить Python 3.10+: `apt install python3 python3-pip python3-venv`

2. **Развертывание кода**
   - [ ] Клонировать репозиторий: `git clone https://github.com/utkabotron/vibe_count.git`
...
3. **Настройка переменных окружения**
   ...
   - [ ] Загрузить Google credentials JSON
   - [ ] Установить права доступа: `chmod 600 .env vibecount-credentials-gsheets.json`

4. **Настройка Cron**
   - [ ] Создать скрипт запуска `run.sh`:
     ```bash
     #!/bin/bash
     cd /path/to/vibe_count
     source venv/bin/activate
     python src/main.py >> logs/cron.log 2>&1
     ```
   - [ ] Сделать исполняемым: `chmod +x run.sh`
   - [ ] Добавить в crontab: `crontab -e`
     ```
     */10 * * * * /path/to/vibe_count/run.sh
     ```
   - [ ] Проверить cron: `crontab -l`

5. **Настройка логирования**
   - [ ] Создать папку для логов: `mkdir logs`
   - [ ] Настроить ротацию логов (logrotate)
   - [ ] Настроить уведомления об ошибках (опционально)

6. **Проверка работы**
   - [ ] Загрузить тестовый файл в `/Входящие` на Яндекс.Диске
   - [ ] Дождаться срабатывания cron (до 10 минут)
   - [ ] Проверить логи: `tail -f logs/cron.log`
   - [ ] Проверить результат в Google Sheets

**Длительность:** 3-4 часа
**Зависимости:** Этап 8

**Примечание:** Альтернативно можно использовать Docker для изоляции окружения:
```dockerfile
...
1. **Настройка мониторинга**
   - [ ] Настроить логирование в файлы с ротацией
   - [ ] Создать скрипт для анализа логов:
     ```bash
     # Количество обработанных файлов за день
     grep "Файл перемещен в архив" logs/cron.log | wc -l

     # Количество ошибок
     grep "ERROR" logs/cron.log | wc -l
     ```
   - [ ] Настроить email уведомления об ошибках (через cron MAILTO)
   - [ ] Опционально: установить Grafana + Loki для визуализации логов
   - [ ] Создать простой дашборд для отслеживания метрик:
...
2. **Оптимизация**
   ...
   - [ ] Настроить автоматическую очистку папки temp/

3. **Документация**
   - [ ] Обновить README.md с инструкциями по деплою на Timeweb
   - [ ] Документировать процесс добавления новых типов документов
   - [ ] Создать инструкцию для пользователя (как работать со статусами)
   - [ ] Документировать процедуру восстановления при сбоях
